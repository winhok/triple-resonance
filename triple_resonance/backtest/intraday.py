...
class BacktestResult:
    ...
    def summarize(self) -> str:
        ...
        L.append(f"交易日 {self.n_days} | T 笔数 {self.n_trades} | 有 T 日 {self.n_t_days}")
        ...
        L.append(f"B 80% Base+20%Csh {self.bench_B_ret:+.2%}")
        L.append(f"C 80% Base+20% To {self.bench_C_ret:+.2%}")
...
def run_backtest(stock_symbol: str, spy_symbol: str, store: ParquetStore,
                ...
                provider: Optional[SessionProvider] = None) -> BacktestResult:
    if provider is None:
    ...
    for day in sorted(set(stock_days) & set(spy_days)):
        sday = sorted(stock_days[day], key=lambda b: b.ts)
        pday = sorted(spy_days[day], key=lambda b: b.ts)
        ...
        open_ts = sday[0].ts
        last_ts = sday[-1].ts
        span = last_ts - open_ts
        opening_range_end = open_ts + timedelta(minutes=15)
        entry_cutoff = open_ts + span - timedelta(minutes=30)
        force_flatten = open_ts + span - timedelta(minutes=15)
        ...
        bars_5m = aggregate(sday, 5, session_opens={day: open_ts})
        ...
        for i, bar in enumerate(sday):
            ...
            if ts >= force_flatten and not eng.state.is_flat:
                r = eng.flatten(bar.close, _iso(bar.ts))
            ...
            if (not entry_done) and opening_range_end < ts < entry_cutoff and eng.state.is_flat:
                ...
                if not pday or pday[0].ts > bar.ts:
                    continue
                ctx = build_context(sday[:i + 1], pday[:i + 1], session, as_of=bar.ts)
                b5 = [b for b in bars_5m if b.ts <= bar.ts]
                sig = detect_trend_pullback(ctx, b5, opening_range_end, rs_threshold)
                if sig:
                    entry_px = sig.entry_ref * (1 + slippage)
        ...
        if cur is not None:
            r = eng.flatten(sday[-1].close, _iso(sday[-1].ts))
    ...
    res.t_pnl_total = sum(t.realized for t in all_trades)
    ...
    # ---- 三基准 ----
    first_close = float(stock[0].close)
    last_close = float(stock[-1].close)
    res.bench_A_ret = last_close / first_close - 1.0
    cash = 0.2 * base_shares * first_close
    b_start = 0.8 * base_shares * first_close + cash
    b_end = 0.8 * base_shares * last_close + cash
    res.bench_B_ret = b_end / b_start - 1.0
    c_end = 0.8 * base_shares * last_close + cash + res.t_pnl_total
...
def _finalize(cur: dict, bar: Bar, realized: float, all_trades: List[Trade],
    ...
    R = (realized / cur["init_risk"]) if cur["init_risk"] else 0.0
    ...
    all_trades.append(Trade(
        ...
        entry_px=ep, exit_px=float(exit_px), realized=float(realized),
